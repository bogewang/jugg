...
import com.lframework.starter.web.inner.entity.*;
...
import com.lframework.starter.web.inner.service.system.*;
...
public class DefaultSysSelectorController extends DefaultBaseController {
  ...
  @ApiOperation("用户")
  @GetMapping("/user")
  public InvokeResult<PageResult<SelectorBo>> user(@Valid QuerySelectorVo vo) {

    SysUserSelectorVo userSelectorVo = SysUserSelectorVo.builder().name(vo.getLabel()).build();
    PageResult<SysUser> pageResult = sysUserService.selector(getPageIndex(vo), getPageSize(vo), userSelectorVo);
    List<SysUser> datas = pageResult.getDatas();
    List<SelectorBo> results = null;

    if (!CollectionUtil.isEmpty(datas)) {
      results = datas.stream().map(item -> SelectorBo.builder().label(item.getName()).value(item.getId()).build()).collect(Collectors.toList());
    }
  ...
  public InvokeResult<List<SelectorBo>> loadUser(@RequestBody(required = false) List<String> ids) {
    ...
    List<SelectorBo> results = datas.stream().map(item -> SelectorBo.builder().label(item.getName()).value(item.getId()).build())
        .collect(Collectors.toList());
