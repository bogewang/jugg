...
public class AuthController extends DefaultBaseController {
  ...
  public InvokeResult<LoginBo> info() {
    ...
    AbstractUserDetails user = getRequiredCurrentUser();
  ...
  public InvokeResult<List<MenuBo>> menus() {
    ...
    if (TenantUtil.enableTenant() && user.getTenantId() != null) {
      moduleIds = sysModuleTenantService.getAvailableModuleIdsByTenantId(user.getTenantId());
    }
  ...
  private AbstractUserDetails getRequiredCurrentUser() {
