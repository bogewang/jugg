...
public class WebExceptionHandler {
  ...
  protected void setResponseCode(BaseException e) {
    ...
    if (e != null && e.getCode() != null) {
      if (ResponseConstants.INVOKE_RESULT_FAIL_CODE_AUTH_EXPIRED.equals(e.getCode())) {
        status = HttpStatus.UNAUTHORIZED.value();
      } else if (ResponseConstants.INVOKE_RESULT_FAIL_CODE_ACCESS_DENIED.equals(e.getCode())) {
        status = HttpStatus.FORBIDDEN.value();
      } else if (ResponseConstants.INVOKE_RESULT_FAIL_CODE_INPUT_ERROR.equals(e.getCode())) {
        status = HttpStatus.BAD_REQUEST.value();
      }
    }

    ResponseUtil.getResponse().setStatus(status);
  }
