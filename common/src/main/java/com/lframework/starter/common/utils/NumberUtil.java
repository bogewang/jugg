...
public class NumberUtil {
    ...
    public static final Integer AMT_PRECISION = 6;
    public static final Integer NUM_PRECISION = 2;
    public static final BigDecimal SIX_DECIMAL_ZERO = BigDecimal.ZERO.setScale(AMT_PRECISION);
    ...
    public static BigDecimal getDefaultValue(BigDecimal input) {
    ...
    public static String formatNumber(Number number, int precision) {
        return getNumber(number, precision).toString();
    }
     ...
     * @param num 数量
     * @param price   含税单价
     * @return 验收金额，固定保留 CommonConstant.PRECISION 位小数
     */
    public static BigDecimal calculateAmount(BigDecimal price, BigDecimal num) {
        if (num == null || price == null) {
            return SIX_DECIMAL_ZERO;
        }

        return num.multiply(price).setScale(6, RoundingMode.HALF_UP);
    }
}
